...
export default function AddRole() {
  ...
  useEffect(() => {
    if (params.id) {
      axios.get(`http://localhost:5000/roll/${params.id}`, {
  ...
  function resubmit() {
    ...
    if (name && discription) {
      axios.put("http://localhost:5000/roll/" + params.id, { name, discription }, {
        headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem("auth"))}` }
      }).then((resp) => {
        // setDiscription('')
        if (params.id) {
          setMsg(`Your roll id is: ${params.id}`)
        } else {
          setMsg(`Your roll id is: ${resp.data._id}`)
        }
  ...
  function submit() {
    if (name && discription) {
      axios.post("http://localhost:5000/roll", { name, discription }, {
        headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem("auth"))}` }
      }).then((resp) => {
        setDiscription('')
        setMsg(`Your roll id is: ${resp.data._id}`)
